#!/usr/bin/env python3
# Deep continuous research of Chile prefab home companies using OpenAI
import sys
import os
import json
import csv
...
csv_path = output_dir / 'chile_prefab_core.csv'
...
# Chile regions for comprehensive research
CHILE_REGIONS = [
    ...
    {'code': 'OH', 'name': "Libertador General Bernardo O'Higgins"},
    {'code': 'MA', 'name': 'Maule'},
    {'code': 'NB', 'name': 'Ñuble'},
    {'code': 'BI', 'name': 'Biobío'},
    ...
    {'code': 'AI', 'name': 'Aysén'},
    {'code': 'MG', 'name': 'Magallanes y la Antártica Chilena'}
]
...
def get_companies_for_region_deep(region: Dict, existing_companies: List[str], iteration: int) -> List[Dict]:
    """Deep research: Get additional companies for a region, excluding already found ones"""
    existing_names = ', '.join(existing_companies[:20]) if existing_companies else "none"
    
    prompt = f"""You are a research expert specializing in prefabricated/modular home companies in {region['name']}, Chile.
...
Your task is to find ADDITIONAL companies in {region['name']} that we haven't found yet. Do NOT repeat companies we already have.
...
- Casas prefabricadas manufacturers
- Constructores modulares
- Fabricantes de casas industrializadas
- Constructores de casas premoldeadas
...
[{{"name": "New Company Name", "region": "{region['name']}"}}, {{"name": "Another New Company", "region": "{region['name']}"}}, ...]
...
def get_companies_for_region(region: Dict) -> List[Dict]:
    """Get companies for a specific Chilean region with deep research (up to 15 iterations)"""
    all_found_companies = []
    max_iterations = 15
...
- Include companies with manufacturing facilities in {region['name']}
- Include companies that primarily serve {region['name']} market
- Search thoroughly - there are typically dozens of such companies per region
...
Return ONLY the JSON array, no explanations or additional text."""
    ...
    try:
        ...
        if json_match:
            try:
                ...
                if not companies_list:
                    ...
                    companies_list = get_companies_for_region_alternative(region)
        ...
        print(f"  ❌ Error in initial research for {region['name']}: {error}")
    ...
    for iteration in range(2, max_iterations + 1):
        ...
        new_companies = get_companies_for_region_deep(region, existing_company_names, iteration)
...
def get_companies_for_region_alternative(region: Dict) -> List[Dict]:
    ...
    prompt = f"""List all prefab home companies in {region['name']} region, Chile.
...
- Local prefab builders in {region['name']}
- Regional modular manufacturers in {region['name']}
- Casas prefabricadas manufacturers
- Constructores modulares
- Fabricantes de casas industrializadas
- Constructores de casas premoldeadas
...
def research_company(company_name: str, region: str) -> Dict:
    ...
    prompt = f"""You are a professional researcher gathering detailed information about a prefabricated/modular home company in {region}, Chile.
...
{{
  ...
  "address": "complete address: street number, street name, city, region, postal code, Chile",
  ...
  "min_home_price": minimum starting price in CLP (Chilean Pesos) for their cheapest model (number, base price without land),
  "average_price_sqm": average price per square meter in CLP (Chilean Pesos) across their models (number, calculate from their pricing, convert from sqft pricing if needed)
}}
...
- All prices should be in CLP (Chilean Pesos)
...
def save_progress():
    ...
    backup_path = output_dir / 'chile_progress_backup.json'
...
def main():
    ...
    print('🚀 Starting deep continuous research of Chile prefab home companies...\n')
    print(f"Using OpenAI ({openai_config['name']})\n")
    print(f"Researching {len(CHILE_REGIONS)} Chilean regions one by one\n")
    ...
    try:
        # Process each region
        for i, region in enumerate(CHILE_REGIONS, 1):
            if region['code'] in processed_regions:
                print(f"\n⏭️  Skipping {region['name']} (already processed)")
                continue
            
            print(f"\n{'=' * 60}")
            print(f"🗺️  Region {i}/{len(CHILE_REGIONS)}: {region['name']} ({region['code']})")
            print(f"{'=' * 60}\n")
        ...
        # Also save as JSON
        json_path = output_dir / 'chile_prefab_core.json'
        ...
        print(f"\n🎉 Research complete! Processed {len(all_companies)} companies across {len(CHILE_REGIONS)} regions.")
