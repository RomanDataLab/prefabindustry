#!/usr/bin/env python3
# Deep continuous research of China prefab home companies using OpenAI
import sys
import os
import json
import csv
...
csv_path = output_dir / 'china_prefab_core.csv'
...
# China provinces, municipalities, and special administrative regions for comprehensive research
CHINA_PROVINCES = [
...
def get_companies_for_province(province: Dict) -> List[Dict]:
    """Get companies for a specific Chinese province"""
    prompt = f"""You are a research expert specializing in prefabricated/modular home companies in {province['name']}, China.
...
Return ONLY the JSON array, no explanations or additional text."""
    ...
    try:
        ...
        if json_match:
            try:
                ...
                if not companies_list:
                    ...
                    return get_companies_for_province_alternative(province)
        ...
        print(f"  ❌ Error researching {province['name']}: {error}")
...
def get_companies_for_province_alternative(province: Dict) -> List[Dict]:
    ...
    prompt = f"""List all prefab home companies in {province['name']} province, China.
...
- Clayton Homes (if in {province['name']})
- Champion Homes (if in {province['name']})
- Fleetwood Homes (if in {province['name']})
- Palm Harbor Homes (if in {province['name']})
...
def research_company(company_name: str, province: str) -> Dict:
    ...
    prompt = f"""You are a professional researcher gathering detailed information about a prefabricated/modular home company in {province}, China.
...
{{
  ...
  "address": "complete address: street number, street name, city, province, postal code, China",
  ...
  "min_home_price": minimum starting price in CNY for their cheapest model (number, base price without land),
  "average_price_sqm": average price per square meter in CNY across their models (number, calculate from their pricing, convert from sqft pricing if needed)
}}
...
def save_progress():
    ...
    backup_path = output_dir / 'china_progress_backup.json'
...
def main():
    ...
    print('🚀 Starting deep continuous research of China prefab home companies...\n')
    print(f"Using OpenAI ({openai_config['name']})\n")
    print(f"Researching {len(CHINA_PROVINCES)} China provinces one by one\n")
    ...
    try:
        ...
        for i, province in enumerate(CHINA_PROVINCES, 1):
            if province['code'] in processed_provinces:
                print(f"\n⏭️  Skipping {province['name']} (already processed)")
                continue
            
            print(f"\n{'=' * 60}")
            print(f"🗺️  Province {i}/{len(CHINA_PROVINCES)}: {province['name']} ({province['code']})")
            print(f"{'=' * 60}\n")
        ...
        # Also save as JSON
        json_path = output_dir / 'china_prefab_core.json'
        ...
        print(f"\n🎉 Research complete! Processed {len(all_companies)} companies across {len(CHINA_PROVINCES)} provinces.")
