#!/usr/bin/env python3
# Deep continuous research of Mexico prefab home companies using OpenAI
import sys
import os
import json
import csv
...
csv_path = output_dir / 'mexico_prefab_core.csv'
...
# USA states for comprehensive research
MEXICO_STATES = [
...
def get_companies_for_state_deep(state: Dict, existing_companies: List[str], iteration: int) -> List[Dict]:
    ...
    prompt = f"""You are a research expert specializing in prefabricated/modular home companies in {state['name']}, México.
...
def get_companies_for_state(state: Dict) -> List[Dict]:
    """Get companies for a specific Mexican state with deep research (up to 15 iterations)"""
    all_found_companies = []
    max_iterations = 15
...
def get_companies_for_state_alternative(state: Dict) -> List[Dict]:
    ...
    prompt = f"""List all prefab home companies in {state['name']} state, México.
...
def research_company(company_name: str, state: str) -> Dict:
    ...
    prompt = f"""You are a professional researcher gathering detailed information about a prefabricated/modular home company in {state}, México.
...
{{
  ...
  "address": "complete address: street number, street name, city, state, código postal, México",
  ...
  "min_home_price": minimum starting price in MXN (Mexican Pesos) for their cheapest model (number, base price without land),
  "average_price_sqm": average price per square meter in MXN (Mexican Pesos) across their models (number, calculate from their pricing, convert from sqft pricing if needed)
}}
...
- All prices should be in MXN (Mexican Pesos)
...
def save_progress():
    ...
    backup_path = output_dir / 'mexico_progress_backup.json'
...
def main():
    ...
    print('🚀 Starting deep continuous research of Mexico prefab home companies...\n')
    print(f"Using OpenAI ({openai_config['name']})\n")
    print(f"Researching {len(MEXICO_STATES)} Mexican states one by one\n")
    ...
    try:
        # Process each state
        for i, state in enumerate(MEXICO_STATES, 1):
            if state['code'] in processed_states:
                print(f"\n⏭️  Skipping {state['name']} (already processed)")
                continue
            
            print(f"\n{'=' * 60}")
            print(f"🗺️  State {i}/{len(MEXICO_STATES)}: {state['name']} ({state['code']})")
            print(f"{'=' * 60}\n")
        ...
        # Also save as JSON
        json_path = output_dir / 'mexico_prefab_core.json'
        ...
        print(f"\n🎉 Research complete! Processed {len(all_companies)} companies across {len(MEXICO_STATES)} states.")
