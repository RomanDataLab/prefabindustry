#!/usr/bin/env python3
# Deep continuous research of UAE prefab home companies using OpenAI
import sys
import os
import json
import csv
...
csv_path = output_dir / 'uae_prefab_core.csv'
...
# UAE emirates for comprehensive research
UAE_EMIRATES = [
...
def get_companies_for_emirate_deep(emirate: Dict, existing_companies: List[str], iteration: int) -> List[Dict]:
    """Deep research: Get additional companies for an emirate, excluding already found ones"""
    existing_names = ', '.join(existing_companies[:20]) if existing_companies else "none"
    
    prompt = f"""You are a research expert specializing in prefabricated/modular home companies in {emirate['name']}, United Arab Emirates.
...
Your task is to find ADDITIONAL companies in {emirate['name']} that we haven't found yet. Do NOT repeat companies we already have.
...
- Steel frame home manufacturers
- Concrete prefab builders
- Luxury prefab home companies
...
- Include companies headquartered in {emirate['name']}
- Include companies with manufacturing facilities in {emirate['name']}
- Include companies that primarily serve {emirate['name']} market
- Search in both English and Arabic company names

Return ONLY a JSON array with NEW companies (at least 5-15 if they exist). Format:
[{{"name": "New Company Name", "emirate": "{emirate['name']}"}}, {{"name": "Another New Company", "emirate": "{emirate['name']}"}}, ...]
    ...
    try:
        ...
        if json_match:
            try:
                ...
                companies_list = [{'name': c.get('name', ''), 'emirate': emirate['name']} for c in companies if c.get('name')]
                ...
                companies_list = extract_company_names(response, emirate['name'])
                existing_lower = {name.lower().strip() for name in existing_companies}
                companies_list = [c for c in companies_list if c['name'].lower().strip() not in existing_lower]
                return companies_list
        companies_list = extract_company_names(response, emirate['name'])
...
def get_companies_for_emirate(emirate: Dict) -> List[Dict]:
    """Get companies for a specific UAE emirate with deep research (up to 15 iterations)"""
    all_found_companies = []
    max_iterations = 15
...
Your task is to provide a COMPREHENSIVE list of ALL companies in {emirate['name']} that manufacture, build, or sell prefab homes. This includes:
...
- Include companies with manufacturing facilities in {emirate['name']}
- Include companies that primarily serve {emirate['name']} market
- Search thoroughly - there are typically many such companies per emirate
...
[{{"name": "Company Name", "emirate": "{emirate['name']}"}}, {{"name": "Another Company", "emirate": "{emirate['name']}"}}, ...]
...
Return ONLY the JSON array, no explanations or additional text."""
    ...
    try:
        ...
        if json_match:
            try:
                ...
                if not companies_list:
                    ...
                    companies_list = get_companies_for_emirate_alternative(emirate)
                
                if companies_list:
                    all_found_companies.extend(companies_list)
                    print(f"  ✅ Found {len(companies_list)} companies in initial discovery")
            except json.JSONDecodeError as e:
                print(f"  ⚠️  Error parsing JSON for {emirate['name']}: {e}")
                companies_list = extract_company_names(response, emirate['name'])
                if not companies_list:
                    companies_list = get_companies_for_emirate_alternative(emirate)
                if companies_list:
        ...
        print(f"  ❌ Error in initial research for {emirate['name']}: {error}")
    ...
    for iteration in range(2, max_iterations + 1):
        ...
        new_companies = get_companies_for_emirate_deep(emirate, existing_company_names, iteration)
...
def get_companies_for_emirate_alternative(emirate: Dict) -> List[Dict]:
    """Alternative approach to find companies using a different prompt style"""
    prompt = f"""List all prefab home companies in {emirate['name']} emirate, United Arab Emirates.
...
- Container homes
- Steel frame homes
- Concrete prefab homes

Include companies like:
- Local prefab builders in {emirate['name']}
- Regional modular manufacturers in {emirate['name']}
- Luxury prefab home companies
- Off-site construction companies

Provide at least 10-15 company names if they exist in {emirate['name']}.

Return ONLY a JSON array: [{{"name": "Company Name", "emirate": "{emirate['name']}"}}, ...]"""
    ...
    try:
        ...
        if json_match:
            try:
                companies = json.loads(json_match.group(0))
                return [{'name': c.get('name', ''), 'emirate': emirate['name']} for c in companies if c.get('name')]
            except json.JSONDecodeError:
                return extract_company_names(response, emirate['name'])
        return extract_company_names(response, emirate['name'])
    except Exception as error:
        print(f"  ⚠️  Alternative approach also failed for {emirate['name']}: {error}")
        return []

def extract_company_names(text: str, emirate: str) -> List[Dict]:
    ...
    for line in lines:
        ...
        if match:
            ...
            if len(name) > 2 and len(name) < 100:  # Reasonable company name length
                companies.append({'name': name, 'emirate': emirate})
            continue
        ...
        if re.match(r'^[A-Z][A-Za-z\s&\-\.]+$', line) and 3 <= len(line) <= 80:
            ...
            if not any(word in line.lower() for word in ['company', 'companies', 'list', 'include', 'example']):
                companies.append({'name': line, 'emirate': emirate})
...
def research_company(company_name: str, emirate: str) -> Dict:
    """Research detailed information about a single company"""
    global company_id
    print(f"\n📊 Researching: {company_name} ({emirate})")
    
    prompt = f"""You are a professional researcher gathering detailed information about a prefabricated/modular home company in {emirate}, United Arab Emirates.
...
{{
  ...
  "address": "complete address: building number, street name, area/district, city, emirate, United Arab Emirates",
  ...
  "main_structure_material": "primary construction material: wood/timber/concrete/steel/composite/CLT/cross-laminated timber/container/etc",
  "min_home_price": minimum starting price in AED (UAE Dirham) for their cheapest model (number, base price without land),
  "average_price_sqm": average price per square meter in AED (UAE Dirham) across their models (number, calculate from their pricing, convert from sqft pricing if needed)
}}
...
- All prices should be in AED (UAE Dirham)
...
- For "main_structure_material", use the most common material (wood, concrete, steel, container, etc.)
    ...
    try:
        ...
        if json_match:
            try:
                ...
                return create_default_entry(company_name, emirate)
...
def create_default_entry(company_name: str, emirate: str) -> Dict:
...
def save_progress():
    ...
    backup_path = output_dir / 'uae_progress_backup.json'
...
def main():
    """Main research function - emirate by emirate"""
    global all_companies, company_id
    
    print('🚀 Starting deep continuous research of UAE prefab home companies...\n')
    print(f"Using OpenAI ({openai_config['name']})\n")
    print(f"Researching {len(UAE_EMIRATES)} UAE emirates one by one\n")
    ...
    # Track which emirates have been processed
    processed_emirates = set()
    if has_progress:
        for company in all_companies:
            address = company.get('address') or ''
            region = company.get('region') or ''
            if address or region:
                for emirate in UAE_EMIRATES:
                    if emirate['name'] in address or emirate['name'] in region or emirate['code'] in address:
                        processed_emirates.add(emirate['code'])
                        break
    
    try:
        # Process each emirate
        for i, emirate in enumerate(UAE_EMIRATES, 1):
            if emirate['code'] in processed_emirates:
                print(f"\n⏭️  Skipping {emirate['name']} (already processed)")
                continue
            
            print(f"\n{'=' * 60}")
            print(f"🗺️  Emirate {i}/{len(UAE_EMIRATES)}: {emirate['name']} ({emirate['code']})")
            print(f"{'=' * 60}\n")
            
            # Step 1: Get companies for this emirate
            print(f"🔍 Discovering companies in {emirate['name']}...")
            companies = get_companies_for_emirate(emirate)
            
            if not companies:
                print(f"  ⚠️  No companies found for {emirate['name']}")
                # Rate limiter will handle delays, but add small delay here too
                time.sleep(0.5)
                continue
            
            print(f"  ✅ Found {len(companies)} companies in {emirate['name']}\n")
            
            # Step 2: Research each company for this emirate
            processed = 0
            total = len(companies)
            
            for company in companies:
                try:
                    company_data = research_company(company['name'], company['emirate'])
            ...
            print(f"\n✅ Completed {emirate['name']}: {processed}/{total} companies researched")
            
            # Save progress after each emirate
            save_progress()
            
            # Print rate limit stats after each emirate
            stats = rate_limiter.get_stats()
            print(f"  📊 Rate limit stats: {stats['requests_last_minute']}/min, {stats['requests_last_hour']}/hour, {stats['requests_last_day']}/day")
            
            # Rate limiter handles delays automatically, but add small delay between emirates for safety
            if i < len(UAE_EMIRATES):
                time.sleep(0.5)
        ...
        # Also save as JSON
        json_path = output_dir / 'uae_prefab_core.json'
        ...
        for company in all_companies:
            ...
            if address or region:
                for emirate in UAE_EMIRATES:
                    if emirate['name'] in address or emirate['name'] in region or emirate['code'] in address:
                        emirate_counts[emirate['name']] = emirate_counts.get(emirate['name'], 0) + 1
                        found = True
                        break
            if not found:
                emirate_counts['Unknown'] = emirate_counts.get('Unknown', 0) + 1
        
        print(f"\n📈 Companies by emirate:")
        for emirate, count in sorted(emirate_counts.items(), key=lambda x: x[1], reverse=True):
        ...
        print(f"\n🎉 Research complete! Processed {len(all_companies)} companies across {len(UAE_EMIRATES)} emirates.")
