#!/usr/bin/env python3
# Deep continuous research of USA prefab home companies using OpenAI
import sys
import os
import json
import csv
...
def get_companies_for_state(state: Dict) -> List[Dict]:
    ...
    prompt = f"""You are a research expert specializing in prefabricated/modular home companies in {state['name']}, USA.
...
def get_companies_for_state_alternative(state: Dict) -> List[Dict]:
    ...
    prompt = f"""List all prefab home companies in {state['name']} state, USA.
...
def research_company(company_name: str, state: str) -> Dict:
    ...
    prompt = f"""You are a professional researcher gathering detailed information about a prefabricated/modular home company in {state}, USA.
...
{{
  ...
  "address": "complete address: street number, street name, city, state, ZIP code, USA",
  ...
  "min_home_price": minimum starting price in USD for their cheapest model (number, base price without land),
  "average_price_sqm": average price per square meter in USD across their models (number, calculate from their pricing, convert from sqft pricing if needed)
}}
...
def save_progress():
    ...
    backup_path = output_dir / 'usa_progress_backup.json'
...
def main():
    ...
    print('🚀 Starting deep continuous research of USA prefab home companies...\n')
    print(f"Using OpenAI ({openai_config['name']})\n")
    print(f"Researching {len(USA_STATES)} US states one by one\n")
    ...
    try:
        # Process each state
        for i, state in enumerate(USA_STATES, 1):
            if state['code'] in processed_states:
                print(f"\n⏭️  Skipping {state['name']} (already processed)")
                continue
            
            print(f"\n{'=' * 60}")
            print(f"🗺️  State {i}/{len(USA_STATES)}: {state['name']} ({state['code']})")
            print(f"{'=' * 60}\n")
        ...
        # Also save as JSON
        json_path = output_dir / 'usa_prefab_core.json'
        ...
        print(f"\n🎉 Research complete! Processed {len(all_companies)} companies across {len(USA_STATES)} states.")
