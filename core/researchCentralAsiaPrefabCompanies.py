#!/usr/bin/env python3
# Deep continuous research of Central Asia prefab home companies using OpenAI
import sys
import os
import json
import csv
...
csv_path = output_dir / 'central_asia_prefab_core.csv'
...
# Central Asia countries for comprehensive research
CENTRAL_ASIA_COUNTRIES = [
    {'code': 'KAZ', 'name': 'Kazakhstan', 'currency': 'KZT'},
    {'code': 'UZB', 'name': 'Uzbekistan', 'currency': 'UZS'},
    {'code': 'KGZ', 'name': 'Kyrgyzstan', 'currency': 'KGS'}
]
...
def get_companies_for_country_deep(country: Dict, existing_companies: List[str], iteration: int) -> List[Dict]:
    ...
    prompt = f"""You are a research expert specializing in prefabricated/modular home companies in {country['name']}.
...
Your task is to find ADDITIONAL companies in {country['name']} that we haven't found yet. Do NOT repeat companies we already have.
...
- Сборные дома manufacturers (Russian: sbornye doma)
- Модульные дома builders (Russian: modulnye doma)
- Заводские дома manufacturers (Russian: zavodskie doma)
- Готовые дома builders (Russian: gotovye doma)
...
- Include companies headquartered in {country['name']}
- Include companies with manufacturing facilities in {country['name']}
- Include companies that primarily serve {country['name']} market

Return ONLY a JSON array with NEW companies (at least 5-15 if they exist). Format:
[{{"name": "New Company Name", "country": "{country['name']}"}}, {{"name": "Another New Company", "country": "{country['name']}"}}, ...]
    ...
    try:
        ...
        if json_match:
            try:
                ...
                companies_list = [{'name': c.get('name', ''), 'country': country['name']} for c in companies if c.get('name')]
                ...
                companies_list = extract_company_names(response, country['name'])
                existing_lower = {name.lower().strip() for name in existing_companies}
                companies_list = [c for c in companies_list if c['name'].lower().strip() not in existing_lower]
                return companies_list
        companies_list = extract_company_names(response, country['name'])
...
def get_companies_for_country(country: Dict) -> List[Dict]:
    """Get companies for a specific Central Asian country with deep research (up to 15 iterations)"""
    all_found_companies = []
    max_iterations = 15
...
Your task is to provide a COMPREHENSIVE list of ALL companies in {country['name']} that manufacture, build, or sell prefab homes. This includes:
...
- Include companies with manufacturing facilities in {country['name']}
- Include companies that primarily serve {country['name']} market
- Search thoroughly - there may be dozens of such companies
...
[{{"name": "Company Name", "country": "{country['name']}"}}, {{"name": "Another Company", "country": "{country['name']}"}}, ...]
...
Return ONLY the JSON array, no explanations or additional text."""
    ...
    try:
        ...
        if json_match:
            try:
                ...
                if not companies_list:
                    ...
                    companies_list = get_companies_for_country_alternative(country)
        ...
        print(f"  ❌ Error in initial research for {country['name']}: {error}")
...
def get_companies_for_country_alternative(country: Dict) -> List[Dict]:
    ...
    prompt = f"""List all prefab home companies in {country['name']}.
...
- Local prefab builders in {country['name']}
- Regional modular manufacturers in {country['name']}
- Сборные дома manufacturers (sbornye doma)
- Модульные дома builders (modulnye doma)
- Заводские дома manufacturers (zavodskie doma)

Provide at least 10-15 company names if they exist in {country['name']}.
...
Return ONLY a JSON array: [{{"name": "Company Name", "country": "{country['name']}"}}, ...]"""
    ...
    try:
        ...
        if json_match:
            try:
                companies = json.loads(json_match.group(0))
                return [{'name': c.get('name', ''), 'country': country['name']} for c in companies if c.get('name')]
            except json.JSONDecodeError:
                return extract_company_names(response, country['name'])
        return extract_company_names(response, country['name'])
    except Exception as error:
        print(f"  ⚠️  Alternative approach also failed for {country['name']}: {error}")
        return []
...
def extract_company_names(text: str, country: str) -> List[Dict]:
    ...
    for line in lines:
        ...
        if re.match(r'^[A-ZА-ЯЁ][A-Za-zА-Яа-яЁё\s&\-\.]+$', line) and 3 <= len(line) <= 80:
...
def research_company(company_name: str, country: str, currency_code: str) -> Dict:
    """Research detailed information about a single company"""
    global company_id
    print(f"\n📊 Researching: {company_name} ({country})")
    
    prompt = f"""You are a professional researcher gathering detailed information about a prefabricated/modular home company in {country}.
...
{{
  ...
  "min_home_price": minimum starting price in {currency_code} for their cheapest model (number, base price without land),
  "average_price_sqm": average price per square meter in {currency_code} across their models (number, calculate from their pricing, convert from sqft pricing if needed)
}}
...
- All prices should be in {currency_code}
    ...
    try:
        ...
        if json_match:
            try:
                ...
                # Get country code
                country_code = next((c['code'] for c in CENTRAL_ASIA_COUNTRIES if c['name'] == country), 'UNK')
                ...
                result = {
                    ...
                    'region': country,  # Using country as region for Central Asia
...
def save_progress():
    ...
    backup_path = output_dir / 'central_asia_progress_backup.json'
...
def main():
    ...
    print('🚀 Starting deep continuous research of Central Asia prefab home companies...\n')
    print(f"Using OpenAI ({openai_config['name']})\n")
    print(f"Researching {len(CENTRAL_ASIA_COUNTRIES)} Central Asian countries one by one\n")
    ...
    try:
        ...
        for i, country in enumerate(CENTRAL_ASIA_COUNTRIES, 1):
            if country['name'] in processed_countries:
                print(f"\n⏭️  Skipping {country['name']} (already processed)")
                continue
            
            print(f"\n{'=' * 60}")
            print(f"🗺️  Country {i}/{len(CENTRAL_ASIA_COUNTRIES)}: {country['name']} ({country['code']})")
            print(f"{'=' * 60}\n")
            ...
            for company in companies:
                try:
                    company_data = research_company(company['name'], company['country'], country['currency'])
            ...
            # Rate limiter handles delays automatically, but add small delay between countries for safety
            if i < len(CENTRAL_ASIA_COUNTRIES):
                time.sleep(0.5)
        ...
        # Also save as JSON
        json_path = output_dir / 'central_asia_prefab_core.json'
        ...
        print(f"\n🎉 Research complete! Processed {len(all_companies)} companies across {len(CENTRAL_ASIA_COUNTRIES)} countries.")
