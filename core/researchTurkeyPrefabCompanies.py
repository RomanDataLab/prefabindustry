#!/usr/bin/env python3
# Deep continuous research of Turkey prefab home companies using OpenAI
import sys
import os
import json
import csv
...
csv_path = output_dir / 'turkey_prefab_core.csv'
...
# Turkey provinces for comprehensive research
TURKEY_PROVINCES = [
...
def get_companies_for_province(province: Dict) -> List[Dict]:
    """Get companies for a specific Turkish province"""
    prompt = f"""You are a research expert specializing in prefabricated/modular home companies in {province['name']}, Turkey.
...
def get_companies_for_province_alternative(province: Dict) -> List[Dict]:
    ...
    prompt = f"""List all prefab home companies in {province['name']} province, Turkey.
...
def research_company(company_name: str, province: str) -> Dict:
    ...
    prompt = f"""You are a professional researcher gathering detailed information about a prefabricated/modular home company in {province}, Turkey.
...
{{
  ...
  "address": "complete address: street number, street name, city, province, postal code, Turkey",
  ...
  "min_home_price": minimum starting price in TRY for their cheapest model (number, base price without land),
  "average_price_sqm": average price per square meter in TRY across their models (number, calculate from their pricing, convert from sqft pricing if needed)
}}
...
def save_progress():
    ...
    backup_path = output_dir / 'turkey_progress_backup.json'
...
def main():
    ...
    print('🚀 Starting deep continuous research of Turkey prefab home companies...\n')
    print(f"Using OpenAI ({openai_config['name']})\n")
    print(f"Researching {len(TURKEY_PROVINCES)} Turkey provinces one by one\n")
    ...
    try:
        ...
        for i, province in enumerate(TURKEY_PROVINCES, 1):
            ...
            print(f"\n{'=' * 60}")
            print(f"🗺️  Province {i}/{len(TURKEY_PROVINCES)}: {province['name']} ({province['code']})")
            print(f"{'=' * 60}\n")
        ...
        # Also save as JSON
        json_path = output_dir / 'turkey_prefab_core.json'
        ...
        print(f"\n🎉 Research complete! Processed {len(all_companies)} companies across {len(TURKEY_PROVINCES)} provinces.")
