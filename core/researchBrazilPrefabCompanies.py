#!/usr/bin/env python3
# Deep continuous research of Brazil prefab home companies using OpenAI
import sys
import os
import json
import csv
...
csv_path = output_dir / 'brazil_prefab_core.csv'
...
# Brazil states for comprehensive research
BRAZIL_STATES = [
    {'code': 'AC', 'name': 'Acre'},
...
def get_companies_for_state_deep(state: Dict, existing_companies: List[str], iteration: int) -> List[Dict]:
    """Deep research: Get additional companies for a state, excluding already found ones"""
    existing_names = ', '.join(existing_companies[:20]) if existing_companies else "none"
    
    prompt = f"""You are a research expert specializing in prefabricated/modular home companies in {state['name']}, Brasil.
...
Your task is to find ADDITIONAL companies in {state['name']} that we haven't found yet. Do NOT repeat companies we already have.
...
- Casas pré-fabricadas manufacturers
- Construtores modulares
- Fabricantes de casas industrializadas
- Construtores de casas pré-moldadas
...
[{{"name": "New Company Name", "state": "{state['name']}"}}, {{"name": "Another New Company", "state": "{state['name']}"}}, ...]
...
def get_companies_for_state(state: Dict) -> List[Dict]:
    """Get companies for a specific Brazilian state with deep research (up to 15 iterations)"""
    all_found_companies = []
    max_iterations = 15
...
Return ONLY the JSON array, no explanations or additional text."""
    ...
    try:
        ...
        if json_match:
            try:
                ...
                if not companies_list:
                    ...
                    companies_list = get_companies_for_state_alternative(state)
        ...
        print(f"  ❌ Error in initial research for {state['name']}: {error}")
    ...
    for iteration in range(2, max_iterations + 1):
        ...
        new_companies = get_companies_for_state_deep(state, existing_company_names, iteration)
...
def get_companies_for_state_alternative(state: Dict) -> List[Dict]:
    ...
    prompt = f"""List all prefab home companies in {state['name']} state, Brasil.
...
- Local prefab builders in {state['name']}
- Regional modular manufacturers in {state['name']}
- Casas pré-fabricadas manufacturers
- Construtores modulares
- Fabricantes de casas industrializadas
- Construtores de casas pré-moldadas
...
def research_company(company_name: str, state: str) -> Dict:
    ...
    prompt = f"""You are a professional researcher gathering detailed information about a prefabricated/modular home company in {state}, Brasil.
...
{{
  ...
  "address": "complete address: street number, street name, city, state, CEP (postal code), Brasil",
  ...
  "min_home_price": minimum starting price in BRL (Brazilian Reais) for their cheapest model (number, base price without land),
  "average_price_sqm": average price per square meter in BRL (Brazilian Reais) across their models (number, calculate from their pricing, convert from sqft pricing if needed)
}}
...
- All prices should be in BRL (Brazilian Reais)
...
def save_progress():
    ...
    backup_path = output_dir / 'brazil_progress_backup.json'
...
def main():
    ...
    print('🚀 Starting deep continuous research of Brazil prefab home companies...\n')
    print(f"Using OpenAI ({openai_config['name']})\n")
    print(f"Researching {len(BRAZIL_STATES)} Brazilian states one by one\n")
    ...
    try:
        # Process each state
        for i, state in enumerate(BRAZIL_STATES, 1):
            if state['code'] in processed_states:
                print(f"\n⏭️  Skipping {state['name']} (already processed)")
                continue
            
            print(f"\n{'=' * 60}")
            print(f"🗺️  State {i}/{len(BRAZIL_STATES)}: {state['name']} ({state['code']})")
            print(f"{'=' * 60}\n")
        ...
        # Also save as JSON
        json_path = output_dir / 'brazil_prefab_core.json'
        ...
        print(f"\n🎉 Research complete! Processed {len(all_companies)} companies across {len(BRAZIL_STATES)} states.")
